...
type UploadResult = {
    ...
    conversionErrors?: Array<{ index: number; error: string }>;
};
...
type EmbeddingResult = {
    ...
    totalCount?: number;
    errors?: Array<{ commentary_id: number; error: string }>;
};

export default function CommentaryUploadPage() {
    ...
    const handleFileUpload = async (file: File) => {
        ...
        try {
            ...
            const uploadUrl = `${backendUrl}/api/v1/admin/commentaries/upload-jsonl`;
            ...
            const normalized: UploadResult = {
                ...
                parsedItems: Array.isArray(data?.parsed_items)
                    ...
                    : Number(data?.parsed_items ?? data?.parsedItems ?? 0),
                insertedCount: Number(data?.inserted_count ?? 0),
                skippedCount: Number(data?.skipped_count ?? 0),
                message: data?.message ?? "",
                errors: Array.isArray(data?.errors) ? data.errors : undefined,
                conversionErrors: Array.isArray(data?.conversion_errors) ? data.conversion_errors : undefined,
            };
    ...
    const handleGenerateEmbeddings = async () => {
        ...
        try {
            const backendUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000";
            const embeddingUrl = `${backendUrl}/api/v1/admin/commentaries/enqueue-embeddings`;
    ...
    const pollEmbeddingStatus = async () => {
        const backendUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000";
        const statusUrl = `${backendUrl}/api/v1/admin/commentaries/embedding-status`;
    ...
    return (
        <div className="page-container">
            ...
            <div className="page-header">
                <h1 className="page-title">해설 파일 업로드</h1>
                ...
                <div className={`result-card ${uploadStatus === "success" ? "result-success" : "result-error"}`}>
                    ...
                    <div className="result-body">
                        ...
                        {uploadResult.insertedCount > 0 && (
                            <p><span className="label">새로 삽입</span> {uploadResult.insertedCount}개</p>
                        )}
                        {uploadResult.skippedCount > 0 && (
                            <p><span className="label">업데이트</span> {uploadResult.skippedCount}개</p>
                        )}
                        ...
                        {uploadResult.errors && uploadResult.errors.length > 0 && (
                            <div className="error-list">
                                <p className="error-list-title">파싱 오류</p>
                                {uploadResult.errors.map((err, idx) => (
                                    <p key={idx} className="error-item">라인 {err.line}: {err.error}</p>
                                ))}
                            </div>
                        )}
                        {uploadResult.conversionErrors && uploadResult.conversionErrors.length > 0 && (
                            <div className="error-list">
                                <p className="error-list-title">매칭 오류 (상위 10개)</p>
                                {uploadResult.conversionErrors.map((err, idx) => (
                                    <p key={idx} className="error-item">#{err.index}: {err.error}</p>
                                ))}
                            </div>
                        )}
                    </div>
            ...
            <div className="page-header">
                ...
                <p className="page-desc">업로드된 해설에 대한 임베딩을 생성합니다.</p>
            </div>
                        ...
                        {embeddingResult.errors && embeddingResult.errors.length > 0 && (
                            <div className="error-list">
                                ...
                                {embeddingResult.errors.map((err, idx) => (
                                    <p key={idx} className="error-item">해설 {err.commentary_id}: {err.error}</p>
                                ))}
                            </div>
                        )}
                    </div>
