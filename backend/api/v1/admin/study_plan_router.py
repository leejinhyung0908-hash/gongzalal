...
class GenerateGuestPlanRequest(BaseModel):
    """게스트 AI 학습 계획 생성 요청 (DB 미저장)."""

    question: str = Field(
        default="내 풀이 데이터를 분석해서 학습 계획을 세워줘",
        description="사용자 질문/요청",
    )
    guest_profile: Dict[str, Any] = Field(default_factory=dict, description="게스트 프로필")
    guest_logs: list[GuestSolvingLog] = Field(default_factory=list, description="게스트 풀이 로그")
...
async def generate_study_plan_guest(
    ...
    """게스트용 AI 학습 계획 생성 (프로필+로그 직접 수신, DB 미저장).

    로그인 없이 전달된 프로필과 풀이 로그를 기반으로
    RAG + Gemini 학습 계획을 생성하되, DB에 저장하지 않습니다.
    """
    try:
        ...
        logger.error(f"[StudyPlanRouter] 게스트 AI 학습 계획 생성 실패: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"게스트 학습 계획 생성 실패: {str(e)}")
