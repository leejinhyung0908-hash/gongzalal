...
# -*- coding: utf-8 -*-
"""
메가공 합격수기 데이터를 기반으로 학습 상담(ADVICE) 데이터셋 생성

메가공 합격수기 데이터에서 질문-답변 쌍을 생성하여 ExaOne 모델 학습용 데이터셋을 만듭니다.
"""
...
class MegagongAdviceDatasetGenerator:
    """메가공 학습 상담 데이터셋 생성기"""

    def __init__(self, input_file: str, output_dir: str = "data/success_stories/megagong"):
        ...
        self.subject_detail_areas = {
            ...
            "행정법": ["행정법일반", "행정작용법", "행정구제법"],
            "행정학": ["행정이론", "조직론", "인사행정", "재무행정", "정책학"],
            "회계학": ["재무회계", "원가회계", "관리회계"],
            "형사정책개론": ["형사정책", "범죄학", "형사법"],
    ...
    def generate_qa_pairs(self, story: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if study_hours:
            for template in self.question_templates["study_hours"]:
                qa_pairs.append({
                    "question": template,
                    "answer": f"{exam_context}\n\n평균적으로 하루에 {study_hours} 정도 공부했습니다.",
                ...
                if subject and subject in subject_methods:
                    ...
                    if subject in self.subject_detail_areas:
                        ...
                        for detail_area in detail_areas:
                            if detail_area.lower() in method_text.lower():
    ...
    def save_to_csv(self, qa_pairs: List[Dict[str, Any]], filename: str = "megagong_advice_dataset.csv"):
        """CSV 파일로 저장"""
    ...
    def save_to_jsonl(
        ...
        filename: str = "megagong_advice_dataset.jsonl",
        format_type: str = "both"
    ):
        ...
        if format_type == "both":
            ...
            if chat_data:
                chat_path = self.output_dir / "megagong_advice_dataset_chat.jsonl"
            ...
            if instruction_data:
                instruction_path = self.output_dir / "megagong_advice_dataset_instruction.jsonl"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="메가공 합격수기 기반 학습 상담 데이터셋 생성")
    parser.add_argument("--input", type=str, required=True, help="합격수기 JSON/JSONL/CSV 파일 경로")
    ...
    generator = MegagongAdviceDatasetGenerator(args.input, args.output_dir)
