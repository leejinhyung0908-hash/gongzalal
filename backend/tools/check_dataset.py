...
# JSONL 파일 읽기
data = []
with open('data/success_stories/gongdanki/success_stories_training.jsonl', 'r', encoding='utf-8') as f:
    for line in f:
        try:
...
if len(data) > 1:
    mid_idx = len(data) // 2
    print(f"\n[샘플 2 - 라인 {mid_idx+1}]")
    print(json.dumps(data[mid_idx], ensure_ascii=False, indent=2))
if len(data) > 2:
    print(f"\n[샘플 3 - 라인 {len(data)}]")
